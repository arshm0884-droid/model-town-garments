...
export default function Store() {
  ...
  useEffect(() => {
    try {
      const savedWishlist =
        localStorage.getItem("model-town-wishlist");
      const savedRecent =
        localStorage.getItem(
          "model-town-recently-viewed"
        );
      const savedCart =
        localStorage.getItem("model-town-cart");
  ...
  const sendWhatsAppOrder = () => {
    ...
    const items = cartProducts
      .map(
        (item) =>
          `• ${item.product.name} | Size: ${item.size} | Color: ${item.color} | Qty: ${item.quantity} | ₹${item.finalPrice * item.quantity}`
      )
      .join("\n");
...
Buy More Discount: -₹${buyMoreDiscount}
Coupon: ${
      appliedCoupon
        ? appliedCoupon.code
        : "None"
...
PAYMENT STATUS:
Customer has requested payment confirmation.
